...
work.dir <- "D:/Articles/BioinformaticsAnalysis/MouseLiverSingleCellRNASequencingAnalysis/20260325"
set.seed(2025)
setwd(work.dir)
...
")
    ...
    cat(sprintf("  CCl4Group: %d/%d (%.1f%%)
", ccl4_act, ccl4_total, 100*ccl4_act/ccl4_total))
    cat(sprintf("  ControlGroup: %d/%d (%.1f%%)
", ctrl_act, ctrl_total, 100*ctrl_act/ctrl_total))
    ...
    # Initializing celltype_reannotated column (if not already present)
...
")
  ...
  cat("Generating DE_stress_comprehensive.csv...\n")
  ...
  cat("Generating stress_scores_all_methods.csv...\n")
  ...
  if(length(high_stress_genes) >= 10) {
    ...
    cat("  Warning: Insufficient high-stress genes(<10), Skipping GO analysis \n")
  }
  ...
  cat("Panel E: Zone×Group stress score\n")
  ...
  # ==================== Panel I: 8-gene signature expression ====================
  ...
  # ==================== Combining complete Figure 2 (3-row layout) ====================
  cat("Combining complete Figure 2...\n")
  
  # Row 1: A B C D (zonation and score distribution)
  ...
  # Row 3: I J K (signature expression, GO, KEGG) - 3 panels, leave blank or adjust
  ...
  # Load DEG results (using existing hsc_deg or load from file)
  if(!exists("hsc_deg")) {
    deg_file <- file.path(tables.dir, "Table_DEG_HSC_Act_vs_Qui_CCl4.csv")
    if(file.exists(deg_file)) {
      hsc_deg <- read.csv(deg_file)
      cat("✓ Loaded HSC DEG results from file:", nrow(hsc_deg), "genes\n")
    } else {
      stop("Error: HSC DEG results file not found")
    }
  }
  ...
  # Ensure significance column is correctly created
  hsc_deg <- hsc_deg %>%
  ...
  if(length(available_genes) > 0) {
    ...
    # Backup: create manually using ggplot
    dot_data <- hsc_deg %>% 
  ...
  if(!is.null(gene_conv) && nrow(gene_conv) > 10) {
    cat("  Successfully converted", nrow(gene_conv), "genes\n")
    ...
    cat("  Gene conversion failed or insufficient genes, skipping enrichment analysis\n")
  ...
  if(length(available_core) >= 4) {
    ...
    tryCatch({
      pheatmap::pheatmap(expr_avg, scale = "row", 
                         ...
                         width = 6, height = 8)
    }, error = function(e) cat("  pheatmap save failed (can be ignored)\n"))
  ...
  if(nrow(target_data) > 0) {
    p3i <- ggplot(target_data, aes(x = reorder(gene, Priority), y = avg_log2FC, fill = Druggability)) +
      ...
      geom_text(aes(label = Druggability), hjust = -0.15, size = 3.5, color = "black", fontface = "bold") +
      scale_fill_manual(values = c("***" = "#C0392B", "**" = "#E67E22", "*" = "#F1C40F"),
                        name = "Druggability") +
      coord_flip() +
      expand_limits(y = max(target_data$avg_log2FC) * 1.15) +
      labs(x = "", y = expression(log[2]~Fold~Change), title = "I. Therapeutic Target Priority") +
      fig_theme +
      theme(legend.position = "right")
  } else {
  ...
  # ========== Combining Figure 3 (3x3 layout) ==========
  cat("Combining Figure 3 (3x3 layout)...\n")
  
  # Fix: Use p3a-p3i instead of p5a-p5i
  ...
  # Ensure CellChat is loaded
  if(!requireNamespace("CellChat", quietly = TRUE)) {
    stop("Please install CellChat first: devtools::install_github('sqjin/CellChat')")
  }
  library(CellChat)
  ...
  if(dir.exists(cellchat.dir)) {
    ...
    if(file.exists(ccl4_rds) && file.exists(control_rds)) {
      ...
      if("HSC_Act" %in% cell_types_ctrl) {
        ...
        cat("  ! Control group missing HSC_Act (excluded due to insufficient cell count)\n")
      }
    } else {
      cat("! CellChat RDS files not found\n")
    }
  } else {
  ...
  extract_communication_safe <- function(cellchat_obj, sources = NULL, targets = NULL) {
    ...
    # Method 1: Try using subsetCommunication (v2.0+)
    comm_data <- tryCatch({
      ...
      cat("    subsetCommunication failed:", conditionMessage(e), "\n")
      cat("    Trying alternative method...\n")
      return(NULL)
    })
    
    # Method 2: If method 1 fails, extract directly from @net
    if(is.null(comm_data) || nrow(comm_data) == 0) {
      cat("    Using @net$prob for direct extraction...\n")
      
      if(!"net" %in% slotNames(cellchat_obj) || 
         is.null(cellchat_obj@net$prob)) {
        cat("    ! Cannot extract communication data\n")
        return(data.frame())
      }
  ...
  if(cellchat_available) {
    ...
    cat("\nExtracting Hep→HSC specific communication...\n")
    ...
    if(nrow(hep_hsc_ccl4) > 0) {
      # Key fix: Merge probabilities by source-target to avoid duplicate pairs
      ...
      if(anyDuplicated(hep_hsc_ccl4_clean$pair) > 0) {
        warning("Duplicate pairs still exist, forcing deduplication")
    ...
    # Panel C: Pathway Activity (using Hep→HSC communication strength as alternative)
    if(nrow(hep_hsc_ccl4) > 0) {
      pw_df <- hep_hsc_ccl4 %>%
        group_by(target) %>%
    ...
    # Panel D: HSC Activation State Shift
    cat("Panel D: HSC Activation State Shift...\n")
    ...
    cat("✓ Panel A-D completed\n")
    
  } else {
    cat("! CellChat not available, generating placeholder plot\n")
    empty_plot <- ggplot() + annotate("text", x=0.5, y=0.5, 
  ...
  # Define key ligand-receptor pairs (literature-based Hep→HSC communication)
  key_lr_pairs <- list(
  ...
  cat("Panel E: Ligand-Receptor Heatmap...\n")
  ...
  # Panel H: Global Network Statistics (Fully Fixed Version)
  ...
  if(exists("df_comm_ccl4") && !is.null(df_comm_ccl4) && nrow(df_comm_ccl4) > 0) {
    ...
    cat(sprintf("  Cleaned communication pairs: %d\n", nrow(df_comm_clean)))
    
    if(nrow(df_comm_clean) > 0) {
      # Calculate out-degree - Key fix: Use dplyr:: prefix and convert to data.frame immediately after summarise
      ...
      # Key fix: Explicitly convert to regular data.frame to avoid class attribute issues
      ...
      # Key fix: Explicitly convert to regular data.frame
      in_degree <- as.data.frame(in_degree)
      names(in_degree) <- c("cell_type", "in_strength", "n_sources")
      
      cat(sprintf("  Out-degree cell count: %d, In-degree cell count: %d\n", 
      ...
      cat(sprintf("  Merged data rows: %d\n", nrow(centrality_df)))
      ...
      if(nrow(centrality_df) > 0) {
        ...
        cat("  ✓ Panel H drawn successfully\n")
      } else {
        p4h <- ggplot2::ggplot() + 
  ...
  cat("✓ Panel E-H completed\n")
  
  # ==================== Combining Figure 4 (2×4 layout) ====================
  cat("\nCombining Figure 4 (2×4 layout)...\n")
  ...
  cat("  5. Correct 2×4 layout\n")
  
  # ==================== Step 12: Figure 5 - Therapeutic Target Prioritization (New) ====================
  cat("\n")
  cat(paste(rep("=", 70), collapse = ""), "\n")
  ...
  if(!exists("hsc_deg")) {
    ...
    if(file.exists(deg_file)) {
      ...
      stop("Error: HSC DEG results not found")
    }
  }
  ...
  # Use *** ** * instead of Unicode star symbols
  ...
  p_5a <- ggplot(target_data, aes(x = reorder(gene, priority_score), y = avg_log2FC)) +
    ...
    geom_text(aes(label = druggability), hjust = 1.15, size = 3.5, color = "white", fontface = "bold") +
    scale_fill_manual(values = c("***" = "#C0392B", "**" = "#E67E22", "*" = "#F1C40F"),
                      name = "Druggability") +
    coord_flip() +
  ...
  # Create mechanism diagram (using ggplot to draw flow chart)
  ...
  cat("\n✓ Figure 5 Complete! (New, Therapeutic Target Prioritization, 2 panels)\n")
  cat("Output files:\n")
